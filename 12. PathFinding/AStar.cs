...
namespace _12._PathFinding
{
	internal class AStar
	{
		...
		* 
		* 다익스트라와 비교
		* 다익스트라는 1. 방문하지 않은 정점중 가장 가까운 정점 선택
		* 2. 거쳐가서 더짧으면 갱신
		* 
		* A+은 1. 방문하지 않은 정점중 가장 유망한( 예상 가중치 가장 가까운 )정점 선택
		* 2. 는 똑같
		* 
		*  S(start) 와 G(goal), H(휴리스틱), 총점 f = g+h
		* f 총점 g 실제이동거리 h 예상남은거리
		* 이 f의 점수가 가장 낮은(유망한) 정점을 찾는게 A*알고리즘
		* 
		* 중간에 이동불가 지역 끼면 예상남은거리 늘어남
		* 
		* h 를 구하는 방식은 두가지
		* 맨허튼거리 방식 - 직각으로만 따라간 거리(가로 + 세로)
		* 
		* 유클리드 방식 -  출발점에서 목표로 한방에 대각으로 쭉이은 거리
		*
		* 타일맵에서 유클리드 쓰자고하면대각선 몇칸에 직선 몇칸 이런식으로 한다고 함
		* 타일맵 직선은 10 대각선은 14로 계산
		* 
		************************************************************/
		...
		static Point[] Direction = // x위치 y위치
		{
			new Point(  0, +1 ),            // 상
            new Point(  0, -1 ),            // 하
            new Point( -1,  0 ),            // 좌
